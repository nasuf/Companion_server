...
def test_variable_sections_ordered_slow_to_fast():
    ...
    expected = [
        ...
        "话题上下文", "相关时间记忆",
        # 事件时间线: 只在聚合类时间问题 ("上次…是几个月前") 出现, 每问都不同 ——
        # 必须留在变化区。插进前面的稳定前缀会让每一轮都 cache miss。
        "事件时间线",
        "久远记忆（L3）",
        # 条数变化 (轮级, 图灵测试 y≠上一轮): 变化区最末, 不打穿前面前缀
        "条数变化",
        # 联网结果使用 (仅联网轮出现): 必须紧贴生成位置 — provider 把搜索结果
        # 作为工具输出追加在上下文最末尾, 这段放远了压不住"照榜单念"的倾向.
        # 只在少数轮出现, 且已在变化区之后, 不影响前面的稳定前缀.
        "联网结果使用",
        "刚聊过的作品",
