...
class _FakeRepo:
    def __init__(self):
        self.conditions = [
            {"id": "c1", "short_name": "杯子", "criteria": "拍到杯子"},
            {"id": "c2", "short_name": "天空", "criteria": "拍到天空"},
            {"id": "c3", "short_name": "小路", "criteria": "拍到小路"},
        ]
    ...
    async def memory_brief(self, uid, wid, *, limit=30):
        return ""


def _install(monkeypatch, *, produce_random=0.0, matches):
    fake = _FakeRepo()
    monkeypatch.setattr(recognition, "repo", fake)
    monkeypatch.setattr(recognition, "_fragment_text", AsyncMock(return_value="思绪正文"))
    monkeypatch.setattr(
    ...
    async def _match(_desc, _conds):
        return queue.pop(0) if queue else {"hit": False, "keywords": []}

    monkeypatch.setattr(recognition, "_match_conditions", _match)
    return fake
...
async def test_full_session_ladder_and_cap(monkeypatch):
    fake = _install(
        ...
        matches=[
            {"hit": True, "condition_short_name": "杯子", "keywords": ["咖啡"]},
            {"hit": True, "condition_short_name": "天空", "keywords": ["云"]},
            {"hit": True, "condition_short_name": "小路", "keywords": ["台阶"]},
            {"hit": True, "condition_short_name": "杯子", "keywords": ["杯壁"]},
        ],
    )
...
async def test_probability_not_passed_keeps_condition_untriggered(monkeypatch):
    fake = _install(
        ...
        matches=[
            {"hit": True, "condition_short_name": "杯子", "keywords": ["咖啡"]},  # 0 条 100% 出
            {"hit": True, "condition_short_name": "天空", "keywords": ["云"]},    # 1 条 0.9>0.4 不出
        ],
    )
    assert await _fire(mid="m1") is not None
    assert await _fire(mid="m2") is None
    assert len(fake.fragments) == 1
    # 概率未过：天空条件保持未触发，后续仍可命中
...
async def test_duplicate_fingerprint_not_reproduced(monkeypatch):
    fake = _install(
        ...
        matches=[
            {"hit": True, "condition_short_name": "杯子", "keywords": ["咖啡"]},
            {"hit": True, "condition_short_name": "天空", "keywords": ["咖啡"]},  # 同指纹
        ],
    )
    assert await _fire(mid="m1") is not None
    assert await _fire(mid="m2") is None  # 同内容指纹 → 不重复产出
    assert len(fake.fragments) == 1


async def test_no_conditions_no_fragment(monkeypatch):
    fake = _install(monkeypatch, matches=[])
    fake.conditions = []
