...
【MBTI 派生性格信号】（0-100，由 MBTI 各维度推导）
- 外向度（E）：{lively}（高分者热情开朗、喜欢分享；低分者安静内敛）
- 思考度（T）：{rational}（高分者逻辑清晰、按事实判断；低分者依赖直觉/情感）
- 共情度（F）：{emotional}（高分者共情能力强、善解人意；低分者冷静直接）
- 判断度（J）：{planned}（高分者喜欢规划、有条理；低分者随性自由）
- 知觉度（P）：{spontaneous}（高分者拥抱变化、灵活应变；低分者按部就班）
- 直觉度（N）：{creative}（高分者思维天马行空；低分者脚踏实地）
- 幽默感（E+N）：{humor}（外向 + 直觉的复合信号；高分者风趣幽默；低分者严肃认真）
