...
PROMPT_DEFINITIONS = [
    ...
    PromptDefinition(
        "offline.shooting_items", "线下打卡拍摄物品生成", "线下互动", "线下活动",
        "【工程扩展】打卡闭环. 到达后由地点/场景生成 3-5 个大类拍摄物品, 供识图匹配, 不展示给用户.",
        OFFLINE_SHOOTING_ITEMS_PROMPT,
    ),
    PromptDefinition(
        "offline.fragment_prewrite", "线下思绪碎片预生成", "线下互动", "线下活动",
        "【工程扩展】打卡闭环. 为每个拍摄物品×档位预生成一段 AI 自己的回忆(取材 AI 经历库), 命中后取用.",
        OFFLINE_FRAGMENT_PREWRITE_PROMPT,
    ),
    PromptDefinition(
        "offline.photo_subjects", "线下现场照片主体判定", "线下互动", "线下活动",
        "【工程扩展】打卡闭环. 依据真实视觉描述判定照片主体及置信度, 供与拍摄物品匹配.",
        OFFLINE_PHOTO_SUBJECTS_PROMPT,
    ),
    PromptDefinition(
    ...
    PromptDefinition(
        "offline.gift_candidate_pick", "礼物候选精选", "线下互动", "礼物",
        "【工程扩展】真实电商接入. 关键词搜出多个候选时, 小模型语义复核挑出最适合做礼物的一件.",
        OFFLINE_GIFT_CANDIDATE_PICK_PROMPT,
    ),
    PromptDefinition(
