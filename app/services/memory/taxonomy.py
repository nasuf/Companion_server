...
L1_COVERAGE_EXEMPT: frozenset[tuple[str, str]] = frozenset({
    ...
    ("生活", "提醒"),
    # Part 5 §4.1 重要日期 (婚礼/手术/纪念日等) 由用户日常告知, agent 创建时不预填
    ("生活", "重要日期"),
    # 红包 / 礼物由结构化 offering 管线写入, agent 创建时不预填
    ("生活", "馈赠"),
...
_LIFE_BASE: tuple[str, ...] = (
    "教育", "工作", "旅行", "居住", "健康", "宠物",
    "人际", "技能", "生活", "提醒",
    # Part 5 §4.1 用户/AI 重要日期 (纪念日/婚礼/手术等). LLM 抽取时若用户语气
    # 是"日历事件"且 occur_time 落未来, 应归入此子类; special_dates 当日扫描
    # 走正向 SELECT subCategory='重要日期' AND occurTime.date()==today.
    "重要日期",
    # 用户给 AI 的红包 / 礼物。金额与状态以 user_offerings 为准,
    # 记忆只负责让 agent 能在对话里回忆起这份心意。后续礼物复用同一子类。
    "馈赠",
...
SUBCATEGORY_ALIASES: dict[str, str] = {
    ...
    "重要事件": "其他特殊事件",
    "难忘经历": "其他特殊事件",
    # Part 5 §4.1 "重要日期" — 用户/AI 告知的具体日历事件 (纪念/婚礼/手术 等)
    # 注: "面试"/"考试" 仍归"职业/与经济"/"教育" — 大多数提及不是为了记日期;
    # 让 LLM 看 occur_time + 用户语气是否"日历事件"决定归类 (extraction prompt 范例).
    "纪念日": "重要日期",
    "周年": "重要日期",
    "婚礼": "重要日期",
    "结婚": "重要日期",
    ...
    "开庭": "重要日期",
    "毕业典礼": "重要日期",
    # 生活/馈赠 — 红包与后续礼物共用, 不新开主类 (身份/情绪/偏好/生活/思维 五轴不变)
    "红包": "馈赠",
    "礼物": "馈赠",
    "送礼": "馈赠",
    "转账": "馈赠",
