"""从互动历史里算出**确定性**的行为事实, 供用户画像使用.

记忆记录的是用户**说过**的话。但有一类信息用户从不会说出口, 只体现在行为里 ——
什么时候来找 AI、来的时候情绪如何、习惯长句还是短句、主动搭话理不理。画像是"我对
这个人的整体了解", 缺了这一半是不完整的, 而在用户记忆普遍稀缺时 (生产实测人均 7
条) 这一半往往是信息量更大的那半。

消费方: `portrait._behaviour_section`。

## 为什么完全不含 LLM

这些数字会喂给画像生成的模型, 而模型会**忠实地总结错误的数据**。设计时我自己就踩
了一次: `messages.created_at` 是 `timestamp without time zone` 存 UTC,
`AT TIME ZONE 'Asia/Shanghai'` 会把它**减** 8 小时。据此算出的活跃时段是"凌晨 5 点
是峰值", 真实是 21 点。这种数字进了画像会变成"用户可能作息紊乱", 而整条链路不报
任何错。

所以这一层只做算术, 每条事实都能用构造数据断言精确值; 判断留给下游。

## 这些观察为什么不做成记忆条目

试过, 行不通。写成独立记忆写进检索池后, 72 条真实用户消息里只有 7% 能召回它们,
且召回时多是误配 —— "你平时什么时候工作啊" 召回 "用户习惯在 21 点聊天", 只是"时间"
这个词撞上了。根因是它们是**特质**不是事实: 向量检索按话题相似度建索引, 而
"他习惯用短句"不关于任何话题, 它关于怎么回应所有话题。画像每轮必然注入, 不需要被
检索到, 才是这类信息该待的地方。
