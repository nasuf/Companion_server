...
半衰期 240 天, 由闸门推演选出, 不是拍的。

初值取 180 天 (对齐旧档位公式的大致衰减速度)。存量人设重排到 L2 之后重跑闸门,
180 天在"完全不访问"这个最坏场景的第 180 天出现 -4.2% 的退化: 人设落在 0.72-0.82,
按 180 天半衰期约 140 天就跌破 warm 下行阈值, 而旧策略的分段档位要两年才到。

扫 180/240/300/365 的结果:

    180 天   第 180 天 -4.2%   可检索 1536
    240 天   第 180 天 +19.4%  可检索 4280
    300 天   第 180 天 +25.0%  可检索 4280+
    365 天   同上, 第 365 天再好一些

240 是消除那个坑的最小值。再往上留存收益递减, 而可检索集合只会更大 —— 控制候选集
规模本来就是分层存在的理由, 没必要为了边际收益放弃它。

一年以上两者收敛 (730 天都是 65% 留存 / 约 50 条可检索), 所以这个选择影响的是
半年这个尺度上的体感。对陪伴类产品来说, "忘得太快"比"记得太多"更伤。

改这个常数必须重跑 evals/memory_lifecycle 的三个场景。
"""
