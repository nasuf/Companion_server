...
class Settings(BaseSettings):
    ...
    # - "silent": evaluation, unlock persistence, the achievement page API,
    #   and wallet point sync all keep running; only the unlock *moments*
    #   are muted — chat WS popups, conversation-timeline achievement rows,
    #   and APNs/system push. Switching back to "on" restores chat-timeline
    #   history automatically (unlocked_at/conversation_id were persisted).
    # - "off": emergency kill switch — no evaluation, achievement page
    #   hidden, daily rollup skipped with its checkpoint frozen so a later
    #   re-enable replays missed days.
