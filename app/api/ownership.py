...
admin 角色 (role='admin') 在所有 owner 检查上一律放行 — 后台管理需要看
所有用户的 agent/对话/记忆, 不必给 admin 端再单独造 mirror endpoint.
"""
...
async def require_agent_owner_any_status(
    ...
    """JWT 解码 + agent 查询 + owner 校验 (admin 跳过), 不限 agent.status.
    ...
    if not _is_admin(user) and agent.userId != user.get("sub"):
...
async def require_user_self(
    ...
    """路径/查询参数 user_id 必须等于 JWT 的 sub (admin 跳过).
    ...
    """
    if not _is_admin(user) and user_id != user.get("sub"):
...
async def require_conversation_owner(
    ...
    """JWT 用户必须是 conversation 的 owner (admin 跳过). 返回 Conversation 对象.
    ...
    if not _is_admin(user) and conv.userId != user.get("sub"):
...
async def require_memory_owner(
    ...
    """JWT 用户必须是 memory 的 owner (admin 跳过). 返回 memory 对象.
    ...
    if not _is_admin(user) and m.userId != user.get("sub"):
